...
## 17. 「タスクマネージャー」表記の統一（→ タスク管理アプリ）
> 2026-07-24 (AI作業)

### 依頼内容
- サイト内の「タスクマネージャー」を「タスク管理アプリ」に、「タスクマネージャー改」を「タスク管理アプリ改」に変更する

### 作業内容
- 画面表示テキストと `<title>` のみを対象に全ファイルを確認し、該当箇所を2ファイル・4箇所検出
  - `app4.html`（19行目の見出し、21行目の本文2箇所）
  - `apps.html`（71行目の見出し、73行目の本文1箇所）
- `app4.html` 21行目は「タスクマネージャーアプリ」という既に「アプリ」を含む表現があったため、単純な文字列置換だと「タスク管理アプリアプリ」と重複してしまう箇所だった。個別に対応し重複を回避
- Windowsのプロセス管理ツールを指す文章は見つからなかった
- ファイル名・ディレクトリ名・id/class名・画像パス・hrefは変更なし
- `FIXES.md` 内（本記録の第13項）の「タスクマネージャー」は画面表示テキストではなく作業履歴のため対象外とし、変更していない

---

## 18. img/ 配下のファイル名リネーム（スペース・括弧の除去）
> 2026-07-24 (AI作業)

### 依頼内容
- `img/` 配下でファイル名にスペースや括弧を含むものを、英数字とアンダースコアのみの名前にリネーム（連番はゼロ埋め2桁、例: `app4_img (1).jpg` → `app4_01.jpg`）

### 作業内容
- 対象18ファイルを洗い出し、リネーム前後の対応表を作成した上で `git mv` でリネーム
  - `img/App2_img/app2_img (1〜4).jpg` → `app2_01〜04.jpg`
  - `img/App3_img/KotlinTra_img (1〜4).jpg` → `KotlinTra_01〜04.jpg`
  - `img/App4_img/app4_img (1〜4).jpg` → `app4_01〜04.jpg`
  - `img/topImage/image (1〜6).jpg` → `image_01〜06.jpg`
